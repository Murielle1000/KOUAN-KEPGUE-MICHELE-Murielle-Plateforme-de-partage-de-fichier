    ...
    <x-slot name="header">
        <div>
            ...
            <nav class="fs-breadcrumb"
                 style="margin-top:0.5rem;color:rgba(255,255,255,0.7);">
                ...
                @foreach($breadcrumb as $crumb)
                    ...
                    <a href="{{ route('folders.show', $crumb) }}"
                       style="color:{{ $loop->last ? 'white' : 'rgba(255,255,255,0.85)' }};
                              font-weight:{{ $loop->last ? '600' : '400' }};">
                        {{ $crumb->name }}
                    </a>
                @endforeach
            </nav>
                        ...
                        @foreach($folder->children as $child)
                            <div>
                                <a href="{{ route('folders.show', $child) }}"
                                   class="fs-folder-item">
                                    <div class="fs-folder-icon">📁</div>
                                    <div style="flex:1;min-width:0;">
                                        <div class="fs-folder-name">{{ $child->name }}</div>
                                        <div class="fs-folder-meta">
                                            {{ $child->files()->count() }} fichier(s)
                                        </div>
                                    </div>
                                </a>
                                <div style="display:flex;gap:0.4rem;margin-top:0.4rem;">
                                    <a href="{{ route('folders.edit', $child) }}"
                                       class="fs-btn fs-btn-ghost fs-btn-sm"
                                       style="flex:1;justify-content:center;">
                                        ✏️ Renommer
                                    </a>
                                    <form action="{{ route('folders.destroy', $child) }}"
                                          method="POST"
                ...
                <table class="fs-table">
                    ...
                    <tbody>
                                    ...
                                    <div style="display:flex;gap:0.4rem;flex-wrap:nowrap;">
                                        ...
                                        @if($file->is_public)
                                            <form action="{{ route('files.revoke-share', $file) }}"
                                                  method="POST">
                                                @csrf @method('DELETE')
                                                <button class="fs-btn fs-btn-ghost fs-btn-sm"
                                                        title="Révoquer">🔒</button>
                                            </form>
                                        @else
                                            <form action="{{ route('files.share', $file) }}"
                                                  method="POST">
                                                @csrf
                                                <button class="fs-btn fs-btn-secondary fs-btn-sm"
                                        ...
                                        <form action="{{ route('files.destroy', $file) }}"
                                              ...
                                              onsubmit="return confirm('Supprimer ?')">
                                            @csrf @method('DELETE')
                                            <button class="fs-btn fs-btn-danger fs-btn-sm"
                        ...
                        @empty
                            <tr>
                                <td colspan="5"
                                    style="text-align:center;padding:2.5rem;color:var(--gray-500);">
